...
class ExpanderSimpleGINLayer(nn.Module):
...
class ExpanderSimpleGIN(nn.Module):
    def __init__(self, net_params):
        super(ExpanderSimpleGIN, self).__init__()
        ...
        for layer in range(self.n_layers):
            self.sgin.append(ExpanderSimpleGINLayer(hidden_dim, hidden_dim, neighbor_aggr_type, residual, 0, learn_eps))
            self.linears_prediction.append(ExpanderMultiLinearLayer(n_mlp_layers, hidden_dim, hidden_dim, out_dim,
        ...
        for layer in range(self.n_layers+1):
            if sparse_readout:
                if mlp_readout:
                    self.linears_readout.append(ExpanderMLPReadout(out_dim, n_classes, self.sparsity))
                else:
                    self.linears_readout.append(ExpanderLinearLayer(out_dim, n_classes, self.sparsity))
            else:
                if mlp_readout:
        ...
        if not sparse_readout and not mlp_readout:
