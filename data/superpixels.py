...
def sigma(dists, kth=8):
    ...
    try:
        ...
        # this sigma value is irrelevant
        # since not used for final compute_edge_list
...
def compute_edges_list(A, kth=8+1):
    ...
    if num_nodes > 9:
        ...
        knns = np.tile(np.arange(num_nodes), num_nodes).reshape(num_nodes,
                                                                num_nodes)
        knn_values = A  # NEW

        # removing self loop
        if num_nodes != 1:
            knn_values = A[knns != np.arange(num_nodes)[:, None]]\
                .reshape(num_nodes, -1)  # NEW
            knns = knns[knns != np.arange(num_nodes)[:, None]]\
                .reshape(num_nodes, -1)
...
class SuperPixDGL(torch.utils.data.Dataset):
    def __init__(self,
        ...
        if dataset == 'MNIST':
            ...
            with open(os.path.join(data_dir,
                                   'mnist_75sp_%s.pkl' % split), 'rb') as f:
                self.labels, self.sp_data = pickle.load(f)
                self.graph_labels = torch.LongTensor(self.labels)
        elif dataset == 'CIFAR10':
            self.img_size = 32
            with open(os.path.join(data_dir,
                                   'cifar10_150sp_%s.pkl' % split), 'rb') as f:
                self.labels, self.sp_data = pickle.load(f)
                self.graph_labels = torch.LongTensor(self.labels)
    ...
    def _prepare(self):
        print("preparing %d graphs for the %s set..." % (self.n_samples,
                                                         self.split.upper()))
        self.Adj_matrices, self.node_features,\
            self.edges_lists, self.edge_features = [], [], [], []
        ...
        for index in range(len(self.sp_data)):
            ...
            for src, dsts in enumerate(self.edges_lists[index]):
                # handling for 1 node where
                # the self loop would be the only edge
                # since, VOC Superpixels has
                # few samples (5 samples) with only 1 node
            ...
            # g.edata['feat'] = torch.ones(g.number_of_edges(),
            #                              edge_feat_dim).half()
            g.edata['feat'] = torch.Tensor(self.edge_features[index])\
                .unsqueeze(1).half()  # NEW
...
class DGLFormDataset(torch.utils.data.Dataset):
    """
        DGLFormDataset wrapping graph list and label list
        as per pytorch Dataset.
...
class SuperPixDatasetDGL(torch.utils.data.Dataset):
    def __init__(self, name, num_val=5000):
        ...
        use_coord = True
        self.test = SuperPixDGL("./data/superpixels",
                                dataset=self.name, split='test',
                                use_mean_px=use_mean_px,
                                use_coord=use_coord)

        self.train_ = SuperPixDGL("./data/superpixels",
                                  dataset=self.name, split='train',
                                  use_mean_px=use_mean_px,
                                  use_coord=use_coord)
...
class SuperPixDataset(torch.utils.data.Dataset):
    def __init__(self, name):
        ...
        with open(data_dir+name+'.pkl', "r") as f:
            f = pickle.load(f)
            self.train = f[0]
            self.val = f[1]
            self.test = f[2]
        print('train, test, val sizes :', len(self.train),
              len(self.test), len(self.val))
    ...
    def collate_dense_gnn(self, samples):
        ...
        """
            ...
            we have the adjacency matrix in R^{n x n},
            the node features in R^{d_n} and edge features R^{d_e}.
            Then we build a zero-initialized tensor, say T,
            in R^{(1 + d_n + d_e) x n x n}. T[0, :, :] is the adjacency matrix.
            The diagonal T[1:1+d_n, i, i], i = 0 to n-1,
            store the node feature of node i.
    ...
    def _sym_normalize_adj(self, adj):
        ...
        deg_inv = torch.where(deg > 0, 1./torch.sqrt(deg),
                              torch.zeros(deg.size()))
    ...
    def _add_self_loops(self):
        ...
        self.train = DGLFormDataset(self.train.graph_lists,
                                    self.train.graph_labels)
        self.val = DGLFormDataset(self.val.graph_lists,
                                  self.val.graph_labels)
        self.test = DGLFormDataset(self.test.graph_lists,
