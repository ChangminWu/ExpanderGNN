...
def self_loop(g):
    ...
    # This new edata is not used since this function
    # gets called only for GCN, GAT
...
class TUsDataset(torch.utils.data.Dataset):
    def __init__(self, name, num_split=10):
        t0 = time.time()
        self.name = name
        ...
        self.all = dataset
        self.train = [self.format_dataset([dataset[idx] for idx in
                                           self.all_idx['train'][split_num]])
                      for split_num in range(num_split)]
        self.val = [self.format_dataset([dataset[idx] for idx in
                                         self.all_idx['val'][split_num]])
                    for split_num in range(num_split)]
        self.test = [self.format_dataset([dataset[idx] for idx in
                                          self.all_idx['test'][split_num]])
                     for split_num in range(num_split)]
    ...
    def format_dataset(self, dataset):
        ...
        for graph in graphs:
            ...
            if 'feat' not in graph.edata.keys():
                ...
                graph.edata['feat'] = torch.ones(graph.number_of_edges(),
                                                 edge_feat_dim)
    ...
    def collate_dense_gnn(self, samples):
        ...
        """
            ...
            we have the adjacency matrix in R^{n x n}, the node features
            in R^{d_n} and edge features R^{d_e}.
            Then we build a zero-initialized tensor, say T,
            in R^{(1 + d_n + d_e) x n x n}. T[0, :, :] is the adjacency matrix.
            The diagonal T[1:1+d_n, i, i], i = 0 to n-1, store the node
            feature of node i.
    ...
    def _add_self_loops(self):
        ...
        for split_num in range(self.num_split):
            self.train[split_num].graph_lists =\
                [self_loop(g) for g in self.train[split_num].graph_lists]
            self.val[split_num].graph_lists =\
                [self_loop(g) for g in self.val[split_num].graph_lists]
            self.test[split_num].graph_lists =\
                [self_loop(g) for g in self.test[split_num].graph_lists]

        for split_num in range(self.num_split):
            self.train[split_num] =\
                DGLFormDataset(self.train[split_num].graph_lists,
                               self.train[split_num].graph_labels)
            self.val[split_num] =\
                DGLFormDataset(self.val[split_num].graph_lists,
                               self.val[split_num].graph_labels)
            self.test[split_num] =\
                DGLFormDataset(self.test[split_num].graph_lists,
                               self.test[split_num].graph_labels)
