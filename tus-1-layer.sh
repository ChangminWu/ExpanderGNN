...
do
  ...
  for j in "${actives[@]}"
  do
    python $script --dataset "$i" --out_dir $savedir --experiment "${j}-random-normalize" --model GCN --linear_type "random-normalize" --config "$config_file" --use_gpu $use_gpu --mlp_layers 1 --activation "$j" --order 2 --batch_norm False --init_lr 0.2 --hidden_dim 1000
    python $script --dataset "$i" --out_dir $savedir --experiment "${j}-random" --model GCN --linear_type "random" --config "$config_file" --use_gpu $use_gpu --mlp_layers 1 --activation "$j" --order 2 --batch_norm False --init_lr 0.2 --hidden_dim 1000
