...
from train.train_citation_node_classification import\
...
def train_val_pipeline(MODEL_NAME, dataset, params, net_params, dirs):
        ...
        with tqdm(range(params['epochs'])) as t:
            for epoch in t:
                ...
                epoch_train_loss, epoch_train_acc, optimizer, writer =\
                    train_epoch(model, optimizer, device, graph, epoch,
                                nfeat, efeat, train_mask, labels, writer)
                ...
                epoch_val_loss, epoch_val_acc =\
                    evaluate_network(model, device, graph, epoch,
    ...
    _, test_acc = evaluate_network(model, device, graph, epoch,
                                   nfeat, efeat, test_mask, labels)
    per_split_test_inference_time.\
        append(time.time() - start_inference_test)
    start_inference_train = time.time()
    _, train_acc = evaluate_network(model, device, graph, epoch,
