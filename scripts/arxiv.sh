...
#densities=(0.005 0.01 0.02 0.05 0.08 0.1 0.5 0.9)
#
...
#    for d in "${densities[@]}"
#    do
#      python $script --dataset "$i" --out_dir $savedir --experiment "random-density-${d}" --model "$j" --density "$d" --linear_type "expander" --config "$config_file" --mlp_layers 1 --use_gpu $use_gpu --sampler "random" --batch_norm False --L 2
##      python $script --dataset "$i" --out_dir $savedir --experiment "rotate-density-${d}" --model "$j" --density "$d" --linear_type "expander" --config "$config_file" --mlp_layers 1 --use_gpu $use_gpu --sampler "rotate" --batch_norm False --L 2
#      python $script --dataset "$i" --out_dir $savedir --experiment "expander-density-${d}" --model "$j" --density "$d" --linear_type "expander" --config "$config_file" --mlp_layers 1 --use_gpu $use_gpu --sampler "regular" --batch_norm False --L 2
#    done
#  done
#done
...
#script=main_citation_node_classification.py
#savedir="results/citation-match-hiddims/"
#datasets=("PUBMED" "CORA" "CITESEER")
#for j in "${models[@]}"
#do
#  for i in "${datasets[@]}"
...
#    for h in "${hiddims[@]}"
#    do
#      python $script --dataset "$i" --out_dir $savedir --experiment "random-dim-${h}" --model "$j" --linear_type "expander" --config "$config_file" --mlp_layers 1 --use_gpu $use_gpu --batch_norm False --L 2 --hidden_dim "$h" --out_dim "$h" --sampler random
#    done
#  done
#done
