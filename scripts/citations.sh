...
savedir="results/node-classification/"
...
do
  ...
  do
    ...
    do
      if [ "$i" = "REDDIT" ]
      then
        python $script --dataset REDDIT --out_dir $savedir --experiment "expander-density-${d}" --model "$j" --density "$d" --linear_type "expander" --config "$config_file" --mlp_layers 1 --use_gpu False --dropout 0.5 --epochs 50
      else
    ...
    then
      ...
      then
        ...
        python $script --dataset "$i" --out_dir $savedir --experiment "activations-${a}" --model "Activation${j}" --activation "$a" --linear_type "regular" --config "$config_file" --mlp_layers 1 --use_gpu $use_gpu --init_lr 0.2 --batch_norm False
      fi
      done
    fi
    ...
    then
      ...
      then
        ...
        python $script --dataset "$i" --out_dir $savedir --experiment "simple" --model "Simple${j}" --linear_type "regular" --config "$config_file" --mlp_layers 1 --use_gpu $use_gpu --init_lr 0.2 --batch_norm False
      fi
    fi

    if [ "$i" = "REDDIT" ]
    then
      python $script --dataset REDDIT --out_dir $savedir --experiment "regular" --model "$j" --linear_type "regular" --density 1.0 --config "$config_file" --mlp_layers 1 --use_gpu False --dropout 0.5 --epochs 50
    else
