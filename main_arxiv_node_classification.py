...
from train.train_arxiv_node_classification import train_epoch, evaluate_network
...
def train_val_pipeline(MODEL_NAME, DATASET_NAME, params, net_params, dirs):
    ...
    train_idx, valid_idx, test_idx = split_idx["train"].to(device), split_idx["valid"].to(device),\
    ...
    if 'PNA' in MODEL_NAME:
        D = torch.cat([torch.sparse.sum(g.adjacency_matrix(transpose=True),
                                        dim=-1).to_dense() for g in [graph]])
        net_params['avg_d'] = dict(lin=torch.mean(D),
                                   exp=torch.mean(torch.exp(torch.
                                                            div(1, D)) - 1),
                                   log=torch.mean(torch.log(D + 1)))

    print("Training Nodes: ", train_idx.size(0))
    print("Validation Nodes: ", valid_idx.size(0))
    print("Test Nodes: ", test_idx.size(0))
