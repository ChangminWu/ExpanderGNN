    ...
    try:
        ...
        for split_number in range(params["num_split"]):
            ...
            if 'PNA' in MODEL_NAME:
                D = torch.cat([
                    torch.sparse.sum(trainset[i][0].adjacency_matrix(transpose=True),
                                     dim=-1).to_dense()
            ...
            with tqdm(range(params['epochs'])) as t:
                for epoch in t:
                    ...
                    epoch_val_loss, epoch_val_acc =\
                        evaluate_network(model, device, evaluator, val_loader)
                    _, epoch_test_acc =\
                        evaluate_network(model, device, evaluator, test_loader)
            ...
            _, test_acc = evaluate_network(model, device, evaluator, test_loader)
            per_split_test_inference_time.\
                append(time.time() - start_inference_test)
            start_inference_train = time.time()
            _, train_acc = evaluate_network(model, device, evaluator, train_loader)
            per_split_train_inference_time.\
...
def main():
    ...
    net_params['in_dim'] = dataset[0][0].ndata['feat'].shape[0]
